...
const useFetchEvolution = (id) => {
  ...
  const checkLevelPokemon = (chain) => {
    ...
    if (chain !== undefined) {
      if (!_isEmpty(chain.evolves_to)) {
        level = level + 1;

        if (!_isEmpty(chain.evolves_to[0].evolves_to)) {
          level = level + 1;

          if (!_isEmpty(chain.evolves_to[0].evolves_to[0].evolves_to)) level = level + 1;
        }
      }
    }
  ...
  const proceedLevelPokemon = () => {
    const { evolves_from_species, name: pokemonName } = pokemonSpecies;

    const { chain } = pokemonChain;

    const checkLevel = checkLevelPokemon(chain);

    if (!_isEmpty(pokemonSpecies) && !_isEmpty(pokemonChain)) {
