...
import { Content, Input, Overlay, Button, TextError } from "./PopupCapture.style";
...
const PopupCapture = ({ data, isActive, toggleFunction, children }) => {
  const [textError, setTextError] = useState("");
  const [nickname, setNickname] = useState("");

  const { addPokemonToDeck, pokemonDeck } = useAppContext();
        ...
        <motion.div initial="init" animate="open" exit="close">
          <Overlay variants={overlayVariant}>
            <Content variants={contentVariant}>
              <CgClose onClick={() => toggleFunction()} />

              <h2> Congratulations! </h2>
              <p>
                You just captured <strong>{data.name.replaceAll("-", " ")}</strong>
              </p>
              ...
              <TextError isError={textError !== ""}>{textError}</TextError>
