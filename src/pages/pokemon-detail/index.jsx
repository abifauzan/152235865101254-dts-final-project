...
import { scrollToTop, getThemePokemon } from "../../utils/util";
...
const PokemonDetail = () => {
  ...
  const { data } = useFetchDetail(endpoint);

  const { pokemonLevel, pokemonSpecies } = useFetchEvolution(data.id);
  ...
  }, [data, pokemonTheme.pokemonType, setTheme]);

  useEffect(() => {
    const fetchPokemon = async () => {
      if (pokemonLevel.length > 0) {
        const getPokemon = await Promise.all(
          pokemonLevel.map(async (el) => {
  ...
  const goToPokemon = (index) => {
    if (pokemonName !== pokemons[index].name) {
      setMounted(false);
      setPage("general");
      navigate(`/pokemon/${pokemons[index].name}`);
    }
  };
  ...
  const mobileView = !_isEmpty(data) && (
    <Container>
      <Menu
        slidesPerView="auto"
        spaceBetween={20}
      ...
      >
        ...
        <TypeContainer>
          {data.types.map((el) => (
            <div key={el.type.name} className="type">
              <img src={getThemePokemon(el.type.name).image} alt={`${data.name}-${el.type.name}`} />
        ...
        <TextColumnThree>
          <div>
            ...
            <TextColumnThreeSpan>{data.weight}</TextColumnThreeSpan>
          </div>
          <div>
            <TextColumnThreeSpan>Height</TextColumnThreeSpan>
            <TextColumnThreeSpan>{data.height}</TextColumnThreeSpan>
          </div>
          <div>
            <TextColumnThreeSpan>Base Exp</TextColumnThreeSpan>
            <TextColumnThreeSpan>{data.base_experience}</TextColumnThreeSpan>
          </div>
        </TextColumnThree>
        ...
        {!_isEmpty(pokemonSpecies) && (
          <GridTwoColumn>
            ...
            <GridColumnText className="bold">
              {pokemonSpecies.habitat !== null
                ? pokemonSpecies.habitat.name.replaceAll("-", "")
                : "-"}
            </GridColumnText>
          </GridTwoColumn>
        )}
        ...
        <StatsWrapper>
          {data.stats.map((el, idx) => (
            <React.Fragment key={el.stat.name}>
              <StatsText>{el.stat.name.replaceAll("-", " ")}</StatsText>
              <StatsText className="bold">{el.base_stat}</StatsText>
              <ProgressBar pokemontheme={theme}>
        ...
        {pokemons.length > 0 ? (
          <List>
            {pokemons.length > 0 && pokemonLevel.length === 2 && (
              <ListItem>
                <ItemPokemon onClick={() => goToPokemon(0)}>
                  <div className="image">
                    <img
                  ...
                  <span className="name">{pokemons[0].name}</span>
                </ItemPokemon>
                ...
                <ItemPokemon onClick={() => goToPokemon(1)}>
                  <div className="image">
                    <img
                  ...
                  <span className="name">{pokemons[1].name}</span>
                </ItemPokemon>
              </ListItem>
            )}
            ...
            {pokemons.length > 0 && pokemonLevel.length === 3 && (
              <>
                ...
                <ListItem>
                  ...
                  <ItemPokemon onClick={() => goToPokemon(2)}>
                    <div className="image">
                      <img
                    ...
                    <span className="name">{pokemons[2].name}</span>
                  </ItemPokemon>
                </ListItem>
              </>
            )}
  ...
  const desktopView = !_isEmpty(data) && (
    <Container>
      <DesktopView
            ...
            <DesktopViewCopy>
              ...
              <TypeContainer>
                {data.types.map((el) => (
                  <div key={el.type.name} className="type">
                    <img
                      src={getThemePokemon(el.type.name).image}
                      alt={`${data.name}-${el.type.name}`}
                    />
                    <span>{el.type.name}</span>
                  </div>
                ))}
  ...
  return !_isEmpty(data) && mounted ? (
    <>
      {isMobile ? mobileView : desktopView}
      <PopupCapture isActive={showPopup} toggleFunction={setShowPopup} data={data} />
    </>
  ) : (
    <LoadingPage />
  );
